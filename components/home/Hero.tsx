...
export default function Hero() {
  ...
  const [starPositions, setStarPositions] = useState<{
    small: Array<{ x: number; y: number; delay: number; duration: number }>
    medium: Array<{ x: number; y: number; delay: number; duration: number }>
    large: Array<{ x: number; y: number; delay: number; duration: number }>
  } | null>(null)
  ...
  // Generate star positions on client after mount to avoid SSR hydration mismatches
  useEffect(() => {
    const isMobile = typeof window !== 'undefined' && window.matchMedia('(max-width: 640px)').matches
    const counts = isMobile
      ? { small: 60, medium: 25, large: 10 }
      : { small: 150, medium: 70, large: 30 }

    const generate = () => ({
      small: Array.from({ length: counts.small }, () => ({
        x: Math.random() * 100,
        y: Math.random() * 100,
        delay: Math.random() * 5,
        duration: Math.random() * 3 + 2,
      })),
      medium: Array.from({ length: counts.medium }, () => ({
        x: Math.random() * 100,
        y: Math.random() * 100,
        delay: Math.random() * 7,
        duration: Math.random() * 4 + 3,
      })),
      large: Array.from({ length: counts.large }, () => ({
        x: Math.random() * 100,
        y: Math.random() * 100,
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-4 pt-16 relative overflow-hidden">
      ...
      {/* Starfield with magnetic field effect (rendered only on client) */}
      {starPositions && (
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        <motion.h1
          ...
          className={`font-heading font-bold text-5xl md:text-7xl text-textPrimary mb-6 leading-tight ${locale === 'lo' ? 'font-lao' : ''}`}
        >
          {t('headline')}
        </motion.h1>
